...
class Product(models.Model):
    """가방 상품 (고정 3종 + 이후 확장될 상품 포함)"""
    name = models.CharField(max_length=100)  # 예: Stark Backpack
    subtitle = models.CharField(max_length=200, blank=True)  # 예: 도시 이동에 최적화된 시그니처 백팩
    description = models.TextField(blank=True)
    image = models.ImageField(upload_to='products/', blank=True, null=True)
    is_default = models.BooleanField(default=False)  # True면 "고정 3종"에 해당
